...
(function ( $ ) {
	...
	// Prepopulate for fields via bitly lead endpoint.
	MktoForms2.onFormRender( function(form) {

		// if the forms are already populated by
		// marketo, don't fill
		...
		var token;
		var endpoint = "https://bitly.com/marketo_leads/?cookie_value=";

		try{
			token = readCookie('_mkto_trk').split('token:')[1];
		}catch(e){
		}

		if( token ){
			jQuery.getJSON( endpoint + token,function(response){
				...
				if( typeof response.data.result[0] != "undefined"){
					...
					var fields = { 
						...
						"Title" : data.title,
						"Phone": data.phone,
						"Company_Size__c": data.Company_Size__c,
						"Vertical__c_lead": data.industry
					};
	...
	function BitlyForm( baseUrl, munchkinId, options ){
		...
		this.filter = function( e ){
			...
			e.find("textarea, select, input[type=text], input[type=email], input[type=tel]").each(function(){
				...
				if($(this).hasClass("mktoRequired")) {
					...
					if(window.location.pathname === '/pages/bitly-brand-tools-landing-page-jp') {
						placeholder = placeholder + " (必須)";  
					} else {
			...
			// add privacy link
			//e.find(".button-toolbar")
				// .append('<div class="privacy small flush-bottom" style="padding:10px 0 10px 8px;"><a href="https://bitly.com/pages/privacy" target="_blank">Privacy Policy</a></div>');
		...
		this.finish = function( element, target, values, followUpUrl ){
			...
			if(dataLayer){
				...
				dataLayer.push({'event':'event.FormCompletion.'+slug});
			}
		...
		this.onSubmit = function( form ){
			...
			//add the conversion pixels here
