!function ($) {
	...
	function views_init() {
		...
		if($.bitly.stage.hasClass("get-started-with-bitly-brand-tools")) {
			...
			view_get_started_with_bitly_brand_tools_init();
                ...
                if($.bitly.stage.hasClass("bitly-brand-tools-landing-page-jp")) {

                        view_get_started_with_bitly_brand_tools_init();

                }

                if($.bitly.stage.hasClass("get-started-bitly-enterprise")) {
		...
		if($.bitly.stage.hasClass("type-landing")) {
			
			view_type_landing_init();
	...
	/* --------------------------------------------------
	Initialize: View (Landing Pages)
	-------------------------------------------------- */
	
	function view_type_landing_init() {
		...
		function fix_content_height(){
		    ...
		    if (responsive_viewport >= 768) {
				...
				$("#page-content-sidebar-bg")
					.css("min-height",$("#page-content-bg").outerHeight());
